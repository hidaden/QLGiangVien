...
class TrangChu_Model
{
    ...
    {
        $querry = $this->conn->prepare("INSERT INTO nguoidung (taikhoan,matkhau) VALUES (?, ?)");
        $querry->bind_param("ss", $taikhoan, $matkhau);
        if ($querry->execute()) {
    ...
    {
        $querry = $this->conn->prepare("DELETE FROM nguoidung WHERE taikhoan = ?");
        $querry->bind_param("s", $taikhoan);
        return $querry->execute();
    }
    public function edit_taikhoan($tenchinhsuaFirst, $tenchinhsuaAfter)
    {
        // Chuẩn bị câu lệnh SQL với tham số
        $querry = $this->conn->prepare("UPDATE nguoidung SET taikhoan = ? WHERE taikhoan = ?");
    ...
    public function edit_matkhau($mkchinhsuaFirst, $mkchinhsuaAfter)
    {
        // Chuẩn bị câu lệnh SQL với tham số
        $querry = $this->conn->prepare("UPDATE nguoidung SET matkhau = ? WHERE matkhau = ?");

        // Liên kết các tham số với giá trị
        $querry->bind_param("ss", $mkchinhsuaAfter, $mkchinhsuaFirst); // "ss" chỉ ra rằng cả hai tham số đều là chuỗi
